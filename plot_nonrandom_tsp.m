...
r_lu(1) = param.r_ia-param.t_i;
r_ap(1) = param.r_ia+param.t_a;
r_ext(1) = r_ap(1)+0.88;
k_i(1,:) = [0.6 1.5];
q_l(1,:) = [0.6 0.9];

param = declare_fixed_parameters(1==1);
r_lu(2) = param.r_ia-param.t_i;
r_ap(2) = param.r_ia+param.t_a;
r_ext(2) = r_ap(2)+0.88;
k_i(2,:) = [1.2 2.8];
q_l(2,:) = [0.2 0.8];
...
text(0.26,0.75,{['$k_i = ' num2str(k_i(1,2)) '\,10^{-18}\,{\rm m}^2$ (healthy)'];['$' num2str(k_i(2,2)) '\,10^{-18}\,{\rm m}^2$ (athero.)']},...
...
text(0.26,0.25,{['$k_i = ' num2str(k_i(1,1)) '\,10^{-18}\,{\rm m}^2$ (healthy)'];['$' num2str(k_i(2,1)) '\,10^{-18}\,{\rm m}^2$ (athero.)']},...
    'FontSize',1/AR*15,'Interpreter','latex','HorizontalAlignment','right','VerticalAlignment','top');
...
text(0.82,0.75,{['$\tilde{q}_{\ell} = ' num2str(q_l(1,2)) '\,10^{-11}\,{\rm m}^2$/s (healthy)'];['$' num2str(q_l(2,2)) '\,10^{-11}\,{\rm m}^2$/s (athero.)']},...
    'FontSize',1/AR*15,'Interpreter','latex','HorizontalAlignment','right','VerticalAlignment','top');
plot(0.54+0.025,0.17,'Marker',ms{1},'color',mean(col,1),'MarkerSize',1/AR*10,'linew',1);
text(0.82,0.25,{['$\tilde{q}_{\ell} = ' num2str(q_l(1,1)) '\,10^{-11}\,{\rm m}^2$/s (healthy)'];['$' num2str(q_l(2,1)) '\,10^{-11}\,{\rm m}^2$/s (athero.)']},...
    'FontSize',1/AR*15,'Interpreter','latex','HorizontalAlignment','right','VerticalAlignment','top');
...
for j = 1:2
    ...
    for i = 1:3
        yl = get(ax(i,j),'YLim');
        if i==1; text(ax(i,j),(xl(1)+param.r_ia)/2,yl(1)+0.95*diff(yl),{'inner';'layers'},'fontsize',1/AR*15,'Interpreter','latex','HorizontalAlignment','center','VerticalAlignment','top'); end
        line(ax(i,j),param.r_ia*[1 1],yl,'color','k');
        if i==1; text(ax(i,j),(param.r_ia+r_ap(j))/2,yl(1)+1*diff(yl),str_a{j},'fontsize',1/AR*15,'Interpreter','latex','HorizontalAlignment','center','VerticalAlignment','top'); end
        line(ax(i,j),r_ap(j)*[1 1],yl,'color','k');
